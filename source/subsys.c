...
#else
#define SUBSYS_DEBUG(fmt, ...) do {} while(0)
#endif
...
static int send_command_and_receive(subsys_handle_t handle, const char* command, 
    ...
    SUBSYS_DEBUG("发送命令: %s", full_command);
    ...
    // 读取响应
    ssize_t bytes_read = read(handle->fd, response, response_size - 1);
    if (bytes_read <= 0) {
        snprintf(handle->last_error, sizeof(handle->last_error), 
                "读取响应失败: %s", strerror(errno));
        pthread_mutex_unlock(&handle->lock);
        return -1;
    }
...
const char* subsys_device_name(subsys_device_t device) {
    switch (device) {
        case SUBSYS_DEVICE_PUMP: return "气泵";
        case SUBSYS_DEVICE_LASER: return "激光";
        case SUBSYS_DEVICE_HEATER1: return "加热器1";
        case SUBSYS_DEVICE_HEATER2: return "加热器2";
        default: return "未知设备";
    }
}
...
const char* subsys_status_string(subsys_device_status_t status) {
    switch (status) {
        case SUBSYS_STATUS_OFF: return "关闭";
        case SUBSYS_STATUS_ON: return "开启";
        case SUBSYS_STATUS_ERROR: return "错误";
        case SUBSYS_STATUS_UNKNOWN: return "未知";
        default: return "无效状态";
    }
}
